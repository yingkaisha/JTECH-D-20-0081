...
for r in range(R18):
    ...
    # loop over members
    grid_boost_flat = grid_boost.reshape(L2, 64, 2*ens)
    for i in range(ens):
        # get model
        model_temp = model_member[i]
        cate_temp_test  = model_temp.predict(np.squeeze(grid_boost_flat[..., 2*i])) #[..., None]
        # RUC
...
# ===== Save data ===== #
#
name = eval_dir+'EVAL_QC_MLP_boost.hdf'
