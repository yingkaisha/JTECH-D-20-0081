...
@nb.njit()
def bch_precip_resample(bch_tuple, sec_ref, gap=30*60):
    '''
...
# --------------------------------------------------- #
with pd.HDFStore(BACKUP_dir+'BCH_30min_2020_10_28.hdf', 'w') as hdf_temp:
    for j, code in enumerate(stn_code):
        try:
            with pd.HDFStore(BCH_file, 'r') as hdf_temp2:
        ...
        bch_tuple = ((temp_pd['datetime'].values.astype('O')/1e9).astype(int),
                     temp_pd['PREC_INST_RAW'].values,
                     temp_pd['PREC_INST_QC'].values,
                     temp_pd['FREQ'].values
                    )
        raw_hourly, qc_hourly = bch_precip_resample(bch_tuple, sec_ref, gap=30*60) # 24*60*60 for daily
        ...
        temp_pd2['PREC_INST_QC'] = qc_hourly
        temp_pd2['PREC_INST_RAW'] = raw_hourly
