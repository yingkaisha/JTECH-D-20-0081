...
# OLD resample scheme
# def BCH_PREC_resample(obs, sec_obs, date_start, sec_ref_min, sec_ref_max, period_out=60*60):
#     '''
#     Coverting NRT bucket heights into precipitation rate [mm/period_out]
#     The same as BC Hydro resampling scheme. 
    
#     Input
#     ----------
#         obs: observed bucket height
#         sec_obs: observed seconds relative to 1970-1-1
#         date_start: datetime of the first obs
#         sec_ref_min, sec_ref_max: min and max seconds to be resampled.
#             * sec_ref_max is not covered, i.e., [min, max)
#         period_out: seconds of resampling period, default is hourly.

#     Output
#     ----------
#         Resampled precipitation rate of mm/period_out
#         A list of datetimes as reference
        
#         * Missing vals are filled with nan
        
#     '''

#     # time difference
#     diff_sec = np.diff(sec_obs)

#     # thrown the first starting time, preserve obs time as ending time
#     sec_obs = sec_obs[1:] 

#     # bucket height difference
#     diff_obs = np.diff(obs)

#     # neg diff means evaporation or bucket dumping
#     diff_obs[diff_obs<0] = 0

#     # converting bucket height diff to precip rate [mm/sec]
#     precip = diff_obs/diff_sec

#     # precip rate interpolation
#     ## expanding "sec_ref" to range of seconds
#     L_raw = len(sec_obs)
#     base_sec = np.arange(sec_ref_min, sec_ref_max+1, 1)

#     ## for each sec within "base_sec", search its closest available obs
#     ind_base = np.searchsorted(sec_obs, base_sec, 'left')

#     ## handling edged vals
#     ind_base[ind_base >= L_raw] = L_raw-1

#     ## Resample obs time and obs preiod to the "base_sec" 
#     precip_on_base = precip[ind_base]
#     sec_on_base = sec_obs[ind_base]
#     diffsec_on_base = diff_sec[ind_base]

#     flag_nan = np.logical_not(
#         np.logical_and(
#             np.logical_and(sec_on_base-base_sec<=diffsec_on_base, sec_on_base-base_sec>=0), diffsec_on_base<=2*period_out))

#     precip_on_base[flag_nan] = np.nan

#     # precip rate resampling
#     L = len(precip_on_base)
#     L_fix = (L//period_out)*period_out
    
#     # time consuming if period is samll
#     precip_out = np.sum(precip_on_base[:L_fix].reshape(-1, period_out), axis=1)
    
#     date_ref = [date_start + timedelta(seconds=x) for x in range(0, sec_ref_max-sec_ref_min, period_out)]
    
#     return precip_out, date_ref
